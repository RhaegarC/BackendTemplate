...
| --- | --- | --- |
| `Tmp.Model` | Entities, DTOs, constants, attributes. No behaviour. | *nothing* |
| `Tmp.Interface` | The contracts: repository and service interfaces, shared abstractions. | `Tmp.Model` |
| `Tmp.Service` | Business logic. Implements `Tmp.Interface` service contracts. | `Tmp.Interface`, `Tmp.Model` |
| `Tmp.Repository` | Data access. Implements `Tmp.Interface` repository contracts. | `Tmp.Interface`, `Tmp.Model` |
| `Tmp.Api` | Composition root: hosting, DI wiring, controllers, middleware, configuration. | all of the above |
| `Tmp.Tests` | xUnit tests. | `Tmp.Api`, `Tmp.Repository`, `Tmp.Interface`, `Tmp.Model` |

Dependencies point **inward**. `Tmp.Interface` is the pivot: both `Service` and `Repository`
depend on it because both *implement* it, and neither knows the other exists. `Tmp.Api` is the
...
```
        Tmp.Api  ──────────────► everything
           │
           ├──► Tmp.Service ──┐
           │                  ├──► Tmp.Interface ──► Tmp.Model
           └──► Tmp.Repository┘
```

### Rules

- **`Tmp.Model` must not gain a project reference.** It is the bottom of the graph. The
  moment it depends on something, every project depends on that something.
- **`Tmp.Model` must not gain a package reference to a framework.** No EF Core, no ASP.NET
  Core. It is plain data, which is why it can be referenced from anywhere.
- **`Tmp.Service` must not reference `Tmp.Repository`.** A service depends on
  `Tmp.Interface`, never on a concrete data-access type. If a service needs data, it asks for
  an interface.
- **`Tmp.Repository` must not reference `Tmp.Service`.** Same rule, other direction.
- **Only `Tmp.Api` reads configuration.** See section 6.
- Place a contract in `Tmp.Interface` under the folder matching its kind — `Repository/`,
...
- **Primary constructors** for dependency injection; assign to a `private readonly` field only
  ...
  ```csharp
  public sealed class OrderRepository(TmpContext context) : DatabaseRepository(context), IOrderRepository
  { }
  ```
...
**1. The entity** — `Tmp.Model/DatabaseEntity/Order.cs`
...
**2. The mapping** — `Tmp.Repository/TmpContext.cs`
...
**3. The contract** — `Tmp.Interface/Repository/IOrderRepository.cs`
...
**4. The implementation** — `Tmp.Repository/OrderRepository.cs`
...
**5. Registration** — `Tmp.Repository/PersistenceExtensions.cs`

```csharp
services.AddScoped<IOrderRepository, OrderRepository>();
```

**6. The service contract and implementation** — under `Tmp.Interface/Service/` and
`Tmp.Service/`. Controllers never see a repository; they see a service.

**7. The controller** — `Tmp.Api/Controllers/OrderController.cs`
...
every `EntityBase` type in `TmpContext.ApplySoftDeleteFilter`, hides those rows from ordinary
...
- **Read configuration only at the composition root** (`Program.cs`) and pass *resolved
  ...
  This is what keeps `Tmp.Repository` config-agnostic and testable.
...
Inject `IUserContextService` (`Tmp.Interface/Infrastructure/`). It exposes `EntraObjectId`,
...
Tests live in `Tmp.Tests` and run with `dotnet test`. **A behaviour change without a test is
...
**Build and test** — builds `Tmp.slnx` and runs the suite.
...
7. **`Tmp.Api.http` requests `/weatherforecast/`,** which does not exist in this solution.
